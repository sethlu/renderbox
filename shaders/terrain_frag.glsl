...
void main() {
    ...
    // Generate noise
    float smallNoise = snoise(vertexWorldPosition.xy * 2);
    float smallNoise2 = snoise((vertexWorldPosition.xy + vec2(1)) * 2);
    ...
    float gradient = 5000;
    vec3 vertexColor = mix(vec3(0.95), vec3(1), clamp((N.z - 0.8) * gradient + 0.5, 0, 1));
